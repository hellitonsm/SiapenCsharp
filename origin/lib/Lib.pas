...
uses Windows, Controls, msxmldom, WinSock, DBXpress, DBCtrls, Grids, DBGrids,
...
//  DB: TDBClass;
  ...
  PERMISSAO_PERFILUSUARIO, PERMISSAO_ADVOGADO, PERMISSAO_VISITANTETRAFICO, PERMISSAO_FACCAO, PERMISSAO_CTC, PERMISSAO_CORRESPONDENCIA : string;
  PERMISSAO_AGENDAATENDIMENTO, PERMISSAO_LIVROREVISTA, PERMISSAO_PAVILHAO, PERMISSAO_GALERIA, PERMISSAO_SOLARIO, PERMISSAO_CELA : string;
  PERMISSAO_CONDICAOINTERNO, PERMISSAO_CIDADE, PERMISSAO_RACA, PERMISSAO_ESCOLARIDADE, PERMISSAO_NACIONALIDADE, PERMISSAO_PROCEDENCIA : string;
  PERMISSAO_DESTINO, PERMISSAO_FORNECEDOR, PERMISSAO_PROFISSAO, PERMISSAO_FALTADISCIPLINAR, PERMISSAO_SERIEFASEESTUDO : string;
  PERMISSAO_PROCESSOCONDENACAO, PERMISSAO_DETRACAO, PERMISSAO_INTERRUPCAO, PERMISSAO_REMICAO, PERMISSAO_ARTIGOPENAL : string;
  PERMISSAO_VARAEXECUCAO, PERMISSAO_REMEDIO, PERMISSAO_FUNCAOINTERNO, PERMISSAO_CALCULOTRABALHO, PERMISSAO_REMICAOTRABALHO : string;
  PERMISSAO_ROUPAS, PERMISSAO_GRAUPARENTESCO, PERMISSAO_TIPOCONTATO, PERMISSAO_ASSUNTO, PERMISSAO_SITUACAO, PERMISSAO_SETORTRABALHOCAD : string;

  PERMISSAO_EVENTOS, PERMISSAO_SUBEVENTOS, PERMISSAO_MONITORAMENTOEVENTOS : string;

  //Permissões estilo SIM ou NÃO.
  CONFIGURACAO, VISUALIZA_OUTRAS_UP, PERMISSAO_ENTRADAVISITANTE, DEFINE_PERFIL_USUARIO, CONCEDE_PERMISSAO_INDIVIDUAL: string;

  LOGIN_CONECTADO, NOME_PERFILUSUARIO_LOGADO, GLOBAL_SENHA_USUARIO, TIPO_SENHA, senha_padrao, skin_padrao: string;
...
uses DmPrincipal, ConsultaGeral, Aguarde, Mensagem, MensagemViaFastReport,
  ModuloPrincipal, Math;
...
begin
  ...
  try
    if FiltraPorNome then
    begin
      sFiltroPorNome := '';
      if NomeEdit <> nil then
      begin
        if StrToFloatDef(Trim(Copy(TEdit(NomeEdit).Text, 1, 3)), -999) = -999 then
        begin
          if StrToFloatDef(Trim(Copy(TEdit(NomeEdit).Text, 1, 2)), 0) = 0 then
          begin
            sFiltroPorNome := UpperCase(TEdit(NomeEdit).Text);
          end;
        end;
      end;
    end;
    ...
    if NomeCons = 'INTERNO' then
      SQL := ' select interno.nome_interno, interno.rgi PRONTUARIO, ' +
        ...
        '     iif(interno.st=''A'',''ATIVO'', ''INATIVO'') status,' +
        '     interno.em_transito,' +
        '     PROCEDENCIA.PROEDENCIA||''/''||coalesce(PROCEDENCIA.UF,''UF'') AS PROCEDENCIA, ' +
        '     INTERNO.id_interno,' +
        '  interno.ID_INTERNO AS CODIGO  from interno ' +
        ...
        '    join galeria on (interno.idgaleria = galeria.id_galeria)' +
        '    join solario on (interno.idsolario = solario.id_solario)' +
        '    LEFT JOIN PROCEDENCIA ON (INTERNO.ID_PROCEDENCIA = PROCEDENCIA.ID_PROCEDENCIA)' +
        ' where interno.nome_interno is not null' +
        ' and pavilhao.id_up = ' + inttostr(GLOBAL_ID_UP) + 'and interno.st = ''A''' +
        ' ORDER BY INTERNO.NOME_INTERNO ';

    if NomeCons = 'PROCESSO' then
      SQL := ' SELECT ' +
        ' P.IDPROCESSO CODIGO, ' +
        ' P.NPROCESSO, ' +
        ' P.VARA, ' +
        ...
        ' P.TOTAL_PENA_DIA ' +
        ' FROM PROCESSO P ' +
        ' WHERE COALESCE(P.SITUACAOJURIDICA,''INQ'') NOT LIKE ''INQ%'' ';

    if NomeCons = 'CELA' then
      SQL := 'SELECT ID_CELA AS CODIGO, CELA FROM CELA ORDER BY CELA ';

    if NomeCons = 'PROFISSAO' then
      SQL := 'SELECT ID_PROFISSAO AS CODIGO, PROFISSAO FROM PROFISSAO ORDER BY PROFISSAO';

    if NomeCons = 'GALERIA' then
      SQL := 'SELECT ID_GALERIA AS CODIGO, GALERIA FROM GALERIA ORDER BY GALERIA';

    if NomeCons = 'PAVILHAO' then
      SQL := 'SELECT ID_PAVILHAO AS CODIGO, PAVILHAO FROM PAVILHAO ORDER BY PAVILHAO';

    if NomeCons = 'SOLARIO' then
      SQL := 'SELECT ID_SOLARIO AS CODIGO, SOLARIO FROM SOLARIO ORDER BY SOLARIO';

    if NomeCons = 'PROCEDENCIA' then
      SQL := 'SELECT ID_PROCEDENCIA AS CODIGO, PROEDENCIA FROM PROCEDENCIA ORDER BY PROEDENCIA';

    if NomeCons = 'CIDADE' then
      SQL := 'SELECT ID_CIDADE AS CODIGO, CIDADE, UF FROM CIDADE ORDER BY CIDADE';

    if NomeCons = 'NACIONALIDADE' then
      SQL := 'SELECT ID_NACIONALIDADE AS CODIGO, NACIONALIDADE FROM NACIONALIDADE ORDER BY NACIONALIDADE';

    if NomeCons = 'ESCOLARIDADE' then
      SQL := 'SELECT ID_ESCOLARIDADE AS CODIGO, ESCOLARIDADE FROM ESCOLARIDADE ORDER BY ESCOLARIDADE';

    if NomeCons = 'RACA' then
      SQL := 'SELECT ID_RACA AS CODIGO, RACA FROM RACA ORDER BY RACA';

    if NomeCons = 'ADVOGADO' then
      SQL := 'SELECT ID_ADVOGADO AS CODIGO, ADVOGADO FROM ADVOGADO ORDER BY ADVOGADO';

    if NomeCons = 'GRAU_PARENTESCO' then
      SQL := 'SELECT ID_GRAU_PARENTESCO AS CODIGO, GRAU_PARENTESCO FROM GRAU_PARENTESCO ORDER BY GRAU_PARENTESCO';

    if NomeCons = 'DESTINO' then
      SQL := 'SELECT ID_DESTINO AS CODIGO, DESTINO, ENDERECO, FONE FROM DESTINO ORDER BY DESTINO';

    if NomeCons = 'POSTO_TRABALHO' then
      SQL := 'SELECT IDPOSTO_TRABALHO AS CODIGO, POSTO_TRABALHO FROM POSTO_TRABALHO ORDER BY POSTO_TRABALHO';

    if NomeCons = 'FUNCIONARIO' then
      SQL := 'SELECT ID_FUNCIONARIO AS CODIGO, NOME_FUNCIONARIO FROM FUNCIONARIO ORDER BY NOME_FUNCIONARIO';

    if NomeCons = 'FUNCIONARIO' then
      SQL := 'SELECT ID_FUNCIONARIO AS CODIGO, NOME_FUNCIONARIO FROM FUNCIONARIO ORDER BY NOME_FUNCIONARIO';

    if NomeCons = 'VISITANTE' then
      SQL := 'SELECT ID_VISITANTE AS CODIGO, VISITANTE FROM VISITANTE ORDER BY VISITANTE';

    {Quando a SQL e Especificada dentro do Form}
    if not (SqlEspecifica = '') then
    begin
      SQL := SqlEspecifica;
    end;

    {Traz o Resulta Informado}
    with FrmConsultaGeral do
    begin
      if SQL <> '' then
      begin
        NomeConsulta := NomeCons; //usado no filtro do cliente
        ConsInitialize(aTop, aLeft, aWidth, aHeight, SQL, IdBusca, CampoResult, MultiplaSelecao);
        ButtonIncluir.Visible := MostrarBotaoIncluir;
        if ShowModal = mrOk then
        begin
          Result := DsConsulta.DataSet.FieldByName(CampoResult).AsVariant;

          for x := Low(ConsGeral_Ret_Campos) to High(ConsGeral_Ret_Campos) do
          begin
            if ConsGeral_Ret_Campos[x] <> '' then
              ConsGeral_Ret_Campos[x] :=
                DsConsulta.DataSet.FieldByName(ConsGeral_Ret_Campos[x]).AsVariant;
          end;

          {Leva o Resultado para o Edit}
          if (NomeEdit <> nil) and (Result <> null) then
          begin
            if NomeEdit.Enabled then
            begin
              if (NomeEdit is TEdit) then
              begin
                TEdit(NomeEdit).Text := Result;
                ConsultaAberta := True;
                if Assigned(TEdit(NomeEdit).OnExit) then
                  TEdit(NomeEdit).OnExit(nil);
              end
              else if (NomeEdit is TMaskEdit) then
              begin
                TMaskEdit(NomeEdit).Text := Result;
                if NomeEdit.CanFocus then
                  NomeEdit.SetFocus;
              end
              else if (NomeEdit is TDBEdit) then
              begin
                TDBEdit(NomeEdit).Text := Result;
                if NomeEdit.CanFocus then
                  NomeEdit.SetFocus;
                if Assigned(TDBEdit(NomeEdit).OnExit) then
                  TEdit(NomeEdit).OnExit(nil);
              end
              else if (NomeEdit is TDBMemo) then
              begin
                TDBMemo(NomeEdit).Text := Result;
                if NomeEdit.CanFocus then
                  NomeEdit.SetFocus;
              end;
            end;
          end;
          //        Result := True ;   //Fora por que é usado o valor do result no Filtro de Lançamento
        end
        else
        begin
          Result := '';
...
begin
  ...
  with FrmAguarde do
  begin
    //    if FileExists('../imagens/tela_aguarde.png') then
    //      Bitmap.LoadFromFile('../imagens/tela_aguarde.png');
    LblAguarde.Visible := MostraLabelAguarde;
    LblAguarde.Caption := TextoLabel;
    LblAguarde.Top := ProgressBarAguarde.Top - (LblAguarde.Height + 10);
    LblAguarde.Font.Size := TamanhoFonte;
    LblAguarde.Font.Color := clBlack;
    LblAguarde.Anchors := [akLeft, akBottom];
    ProgressBarAguarde.Visible := MostraProgressBar;
    ProgressBarAguarde.Max := ProgressMax;
...
begin
  ...
  begin
    ...
    if MaxPosition > 0 then
      FrmAguarde.ProgressBarAguarde.Max := MaxPosition;

    FrmAguarde.ProgressBarAguarde.StepBy(Count);

    if MostraProgressoLabel then
    begin
      FrmAguarde.Repaint;
      FrmAguarde.ProgressBarAguarde.Refresh;
      FrmAguarde.LblAguarde.Caption := 'Aguarde... (' +
        IntToStr(FrmAguarde.ProgressBarAguarde.Position) + ' de ' + IntToStr(FrmAguarde.ProgressBarAguarde.Max) + ')';
    end;
  end;
end;
...
var
  ...
  Mensagem: TIdMessage;
  TextoHTML: TIdText;
  ConexaoSegura: TIdSSLIOHandlerSocket;
  Anexo: TIdAttachment;
  I: Integer;
begin
  Result := False;
  ...
  ConexaoSegura := TIdSSLIOHandlerSocket.Create(nil);
  Mensagem := TIdMessage.Create(nil);

  {** Define o servidor SMTP **}
  SmtpServer.Host := ServidorSMTP;
  {** Define a porta **}
  SmtpServer.Port := PortaSMTP;
  {** Define usuario de autenticação **}
  SmtpServer.Username := EmailRemetente;
  {** Define senha de conexão **}
  SmtpServer.Password := SenhaRemetente;
  {** Define com login a conexão **}
  SmtpServer.AuthenticationType := atLogin;

  {** Opções da segurança, Gmail e Outros **}
  if AutenticacaoSSL then
  begin
    ConexaoSegura.SSLOptions.Method := sslvSSLv23; //sslvSSLv2, sslvSSLv23, sslvSSLv3, sslvTLSv1
    ConexaoSegura.SSLOptions.Mode := sslmClient;
    ConexaoSegura.SSLOptions.VerifyDepth := 0;
    ConexaoSegura.SSLOptions.VerifyDirs := 'c:\windows\system32\';
    ConexaoSegura.UseNagle := True;

    {** define o tipo de conexão segura **}
    SmtpServer.IOHandler := ConexaoSegura;
  end;

  {** Efetua conexão com servidor de email **}
  SmtpServer.ReadTimeout := 0;
  SmtpServer.Connect();
...
begin
  ...
  begin
    ...
    ProgressBarAguarde.Max := FileLength + 1;
    ProgressBarAguarde.Position := 0;
    ...
    begin
      ...
      ProgressBarAguarde.Position := ProgressBarAguarde.Position + NumRead;
      LblAguarde.Caption := TamanhoArquivoFormatado(ProgressBarAguarde.Position) + ' de ' + TamanhoArquivoFormatado(ProgressBarAguarde.Max);
      lblaguarde.repaint;
    ...
    lblaguarde.repaint;
    (Ds.DataSet.FieldByName(sField) as TBlobField).LoadFromStream(MemoryStream);
    FrmAguarde.close;
    FreeAndNil(FrmAguarde);
...
begin
  ...
  begin
    ...
    while FileLength > 0 do
    begin
      MemoryStream.ReadBuffer(Buffer[0], NumRead);
      FileLength := FileLength - NumRead;
...
begin
  ...
  begin
    if not Assigned(FrmMensagem.MemoMensagem) then
    begin
      FrmMensagem := nil;
      FrmMensagem := TFrmMensagem.Create(Application)
    end;
  ...
  FrmMensagem.MemoMensagem.Lines.Clear;
  FrmMensagem.MemoMensagem.Lines.Add(sMensagem);
  FrmMensagem.Show;
...
begin
  ...
  if not Assigned(FrmMensagemViaFastReport) then
    FrmMensagemViaFastReport := TFrmMensagemViaFastReport.Create(FrmModuloPrincipal)
  else
  begin
    if not Assigned(FrmMensagemViaFastReport.RadioGroupTipo) then
    begin
      FrmMensagemViaFastReport := nil;
      FrmMensagemViaFastReport := TFrmMensagemViaFastReport.Create(FrmModuloPrincipal)
    end;
  end;

  FrmMensagemViaFastReport.Show;
  FrmMensagemViaFastReport.SetFocus;
  FrmModuloPrincipal.BringToFront;
  FrmModuloPrincipal.BtnConfere.SetFocus;
...
{Esta Função mescla as permissões do perfil de usuário com as permissões específicas do usuário.
Diogo G. Lima - 15/04/2013}

function MesclarPermissoes(Permissao1, Permissao2, TipoPermissao: string): string;
var
  permissaoFinal: string;
begin
  permissaoFinal := '';

  //Permissão CEDIR, é a forma que a maioria das permissões utilizam.
  if UpperCase(TipoPermissao) = 'CEDIR' then
  begin

    if ((ContemValor('C', Permissao1)) or (ContemValor('C', Permissao2))) then
    begin
      permissaoFinal := permissaoFinal + 'C';
    end;

    if ((ContemValor('E', Permissao1)) or (ContemValor('E', Permissao2))) then
    begin
      permissaoFinal := permissaoFinal + 'E';
    end;

    if ((ContemValor('D', Permissao1)) or (ContemValor('D', Permissao2))) then
    begin
      permissaoFinal := permissaoFinal + 'D';
    end;

    if ((ContemValor('I', Permissao1)) or (ContemValor('I', Permissao2))) then
    begin
      permissaoFinal := permissaoFinal + 'I';
    end;

    if ((ContemValor('R', Permissao1)) or (ContemValor('R', Permissao2))) then
    begin
      permissaoFinal := permissaoFinal + 'R';
    end;

  end;

  //Permissões 'CEDIRP', é a forma que a permissão do módulo de ocorrências utiliza.
  if UpperCase(TipoPermissao) = 'CEDIRP' then
  begin
    ...
    if ((ContemValor('P', Permissao1)) or (ContemValor('P', Permissao2))) then
    begin
      permissaoFinal := permissaoFinal + 'P';
    end;

  end;

  //Permissões 'SN', são as permissões estilo Sim ou Não (S ou N).
  if UpperCase(TipoPermissao) = 'SN' then
  begin

    if ((ContemValor('S', Permissao1)) or (ContemValor('S', Permissao2))) then
    begin
      permissaoFinal := permissaoFinal + 'S';
    end
    else
    begin
      permissaoFinal := permissaoFinal + 'N';
    end;

  end;

  //Permissões 'AI', são as permissões estilo Ativo ou Inativo (A ou I).
  if UpperCase(TipoPermissao) = 'AI' then
  begin

    if ((ContemValor('A', Permissao1)) or (ContemValor('A', Permissao2))) then
    begin
      permissaoFinal := permissaoFinal + 'A';
    end
    else
    begin
      permissaoFinal := permissaoFinal + 'I';
